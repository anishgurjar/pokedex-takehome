...
**Peer confirmation (feature 3):** Added `POST /sightings/{id}/confirm` and `GET /sightings/{id}/confirmation`, using the existing bearer-JWT principal instead of the old header-based identity idea. The confirmation flow now records who confirmed a sighting and when, rejects self-confirmation and double confirmation in the domain/service layer, and treats campaign-locked sightings as non-mutable. I also added database constraints and a foreign key so confirmed rows cannot exist without valid metadata and cannot point back to the reporting ranger, because these invariants are important enough not to trust only application code. The seed script was updated to generate valid confirmed sightings with a different ranger as confirmer so seeded data continues to satisfy the stricter schema.
